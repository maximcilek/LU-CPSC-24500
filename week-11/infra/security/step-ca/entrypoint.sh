...
declare -ra REQUIRED_INIT_VARS=(DOCKER_STEPCA_INIT_NAME DOCKER_STEPCA_INIT_DNS_NAMES DOCKER_STEPCA_INIT_PASSWORD_FILE)
...
function step_ca_init () {
    ...
    if [ -n "${DOCKER_STEPCA_INIT_PASSWORD_FILE}" ]; then
        echo "[+] Adding password file(s) to args"
        setup_args=("${setup_args[@]}" --password-file "${DOCKER_STEPCA_INIT_PASSWORD_FILE}" --provisioner-password-file "${DOCKER_STEPCA_INIT_PASSWORD_FILE}")
    else
        echo "[-] Password File not provided, checking for plaintext configuration"
        if [ -n "${DOCKER_STEPCA_INIT_PASSWORD}" ]; then
            setup_args=("${setup_args[@]}" --password-file "${DOCKER_STEPCA_INIT_PASSWORD}" --provisioner-password-file "${DOCKER_STEPCA_INIT_PASSWORD}")
        else
            echo "Generating password with urandom (stateless)"
            generate_password > "${STEPPATH}/password"
            generate_password > "${STEPPATH}/provisioner_password"
            setup_args=("${setup_args[@]}" --password-file "${STEPPATH}/password" --provisioner-password-file "${STEPPATH}/provisioner_password")
        fi
    fi
